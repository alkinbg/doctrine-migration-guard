...
final class SqlRiskAnalyzerTest extends TestCase
{
    ...
    public static function riskCases(): iterable
    {
        ...
        yield 'add not null default' => ["ALTER TABLE users ADD status VARCHAR(16) NOT NULL DEFAULT 'active'", Severity::Warning];
        ...
        yield 'update where only in subquery' => ['UPDATE users SET x=(SELECT x FROM source WHERE id=1)', Severity::High];
        ...
        yield 'add foreign key' => ['ALTER TABLE orders ADD FOREIGN KEY (user_id) REFERENCES users(id)', Severity::Warning];
        yield 'add named foreign key' => ['ALTER TABLE orders ADD CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users(id)', Severity::Warning];
        ...
        yield 'multi action alter' => ['ALTER TABLE users ADD last_login_at DATETIME DEFAULT NULL, ADD last_seen_at DATETIME DEFAULT NULL', Severity::Unanalyzed];
        yield 'mixed multi action alter' => ["ALTER TABLE news_posts ADD status VARCHAR(16) NOT NULL DEFAULT 'published', ADD author_id INT DEFAULT NULL", Severity::Unanalyzed];
        yield 'multiple statements' => ['ALTER TABLE users ADD nickname VARCHAR(64) DEFAULT NULL; DROP TABLE audit_log;', Severity::Unanalyzed];
        yield 'comma inside decimal is not multi action' => ['ALTER TABLE prices ADD amount DECIMAL(10,2) DEFAULT NULL', Severity::Info];
        yield 'comma inside index list is not multi action' => ['ALTER TABLE users ADD INDEX idx_names (first_name,last_name)', Severity::Warning];
        yield 'case whitespace variant' => ["  alter\n table users\n add column flag int not null default 0  ", Severity::Warning];
    }
